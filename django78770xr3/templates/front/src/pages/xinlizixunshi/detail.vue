	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'>'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/xinlizixunshi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
				...
				<div class="info">
					<div class="title-item">
						<div class="detail-title">
							{{detail.zixunshixingming}}
						</div>
					</div>
					<div class="item">
						<div class="lable">咨询师账号</div>
						<div class="text "  >{{detail.zixunshizhanghao}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('xinlizixunshi','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('xinlizixunshi','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('xinlizixunshi','私聊')&&detail.id!=mid" @click="chatClick">私聊</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.zixunshixingming;
						...
						if(res.data.data.zixunshizhanghao){
							this.fname = res.data.data.zixunshizhanghao
						}
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/xinlizixunshi', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/xinlizixunshiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此心理咨询师？') .then(_ => {
					this.$http.post('xinlizixunshi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
