<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="180px"
			>
			<el-form-item class="add-item" label="咨询师账号" prop="zixunshizhanghao">
			...
			<el-form-item class="add-item" label="咨询师姓名" prop="zixunshixingming">
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`xinlizixunshi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`xinlizixunshi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
