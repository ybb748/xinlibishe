<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px","background":"#ffffff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","order":"2"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">调查问卷名称</label>
						<el-input v-model="searchForm.name" placeholder="调查问卷名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
			...
			<div :style='{"border":"0px solid #fff","padding":"0","color":"#000","background":"none","borderWidth":"0","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="status" label="调查问卷状态">
						<template slot-scope="scope">
							<el-switch
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('exampaper','调查统计')" type="success" @click="staticsClick(scope.row)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								调查统计
							</el-button>
		...
		<el-dialog :title="staticsTitle" :visible.sync="staticsVisible" width="50%">
			<download-excel class="export-excel-wrapper" :data="staticsList" :fields="json_fields"
				:name="`${staticsTitle}.xls`">
				<!-- 导出excel -->
				<el-button type="success">
					导出
				</el-button>
			</download-excel>
			<div v-for="(item,index) in staticsList" :key='index'>
				<div :id="`statics${index}`" style="width: 100%;height: 300px;"></div>
				<el-table :data="item.numList" style="width: 50%;margin: 0 auto;">
				...
				<el-divider v-if="index != staticsList.length - 1"></el-divider>
			</div>
			<span slot="footer" class="dialog-footer">
				<el-button @click="staticsVisible = false">关闭</el-button>
			</span>
		</el-dialog>
		...
		methods: {
			...
			composeSave() {
				this.$http({
					...
					&&radioNum=${this.composeForm.radioNum}&&multipleChoiceNum=${this.composeForm.multipleChoiceNum}&&determineNum=0
					&&fillNum=0&&subjectivityNum=0`,
					method: "post"
				}).then(({data})=>{
					if(data &&data.code==0){
						this.$message({
			...
			staticsClick(row) {
				this.$http({
					url: `${this.$api.examrecordoptions}`,
					methods: 'get',
					params: {
						page: 1,
						limit: 100,
					...
					this.staticsTitle = `${row.name}统计`
					this.staticsList = data.data.list
					this.staticsVisible = true
					for (let x in data.data.list) {
						data.data.list[x].optionList = JSON.parse(data.data.list[x].options)
						data.data.list[x].numList = []
						data.data.list[x] = this.changeNum(data.data.list[x])
					}
					this.staticsList = data.data.list
					this.$nextTick(() => {
						this.echartChange()
			...
			echartChange() {
				this.$nextTick(() => {
					var staticsChart = null
					var options = {}
					for (let i = 0; i < this.staticsList.length; i++) {
						staticsChart = echarts.init(document.getElementById(`statics${i}`), 'macarons')
						options = {
							title: {
								text: this.changeText(this.staticsList[i].questionname),
								left: 'center'
							},
							tooltip: {
								trigger: 'item',
							...
							series: [{
								type: 'pie',
								radius: '50%',
								data: this.staticsList[i].numList,
								emphasis: {
									itemStyle: {
										shadowBlur: 10,
										shadowOffsetX: 0,
						...
						staticsChart.setOption(options)
					}
				})
			},
			changeText(text){
			...
			// 处理数据
			changeNum(row) {
				for (let x in row.optionList) {
					if (row.optionList[x].code == 'A') {
						row.numList.push({
							name: row.optionList[x].text.split('A.')[1],
							value: row.anum
						})
					} else if (row.optionList[x].code == 'B') {
						row.numList.push({
							name: row.optionList[x].text.split('B.')[1],
							value: row.bnum
						})
					} else if (row.optionList[x].code == 'C') {
						row.numList.push({
							name: row.optionList[x].text.split('C.')[1],
							value: row.cnum
						})
					} else if (row.optionList[x].code == 'D') {
						row.numList.push({
							name: row.optionList[x].text.split('D.')[1],
							value: row.dnum
						})
					}
				}
				row.nums = JSON.stringify(row.numList)
				return row
			},
			contentStyleChange() {
