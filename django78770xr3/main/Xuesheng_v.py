...
def xuesheng_save(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if xuesheng.objects.filter(zhanghao = req_dict['zhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "账号已存在"
            return JsonResponse(msg, encoder=CustomJsonEncoder)
        tablename=request.session.get("tablename")
...
def xuesheng_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if req_dict.get('zhanghao')!=None and xuesheng.objects.exclude(id=req_dict['id']).filter(zhanghao = req_dict['zhanghao']).count()>0:
            msg['code'] = crud_error_code
            msg['msg'] = "账号已存在"
